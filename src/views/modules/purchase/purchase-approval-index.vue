  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="approvalHandleButton" ref="projectUpload">审批</el-button>
      </el-form-item>
      <!-- <el-row>
        <el-col :span="24">
          <el-form-item label="审批人:">
            <el-button @click="()=>{
        this.dataForm.approval = this.$store.state.user.userId;
        this.getDataList();
        }">{{this.$store.state.user.name}}
            </el-button>
            <el-button @click="()=>{
        this.dataForm.approval = '';
        this.getDataList();
        }">全部
            </el-button>
          </el-form-item>
...
import approvalInfo from "./purchase-approval-info";
...
export default {
  ...
  methods: {
    ...
    approvalHandle(row) {
      if(row.approval!=this.$store.state.user.userId){
        return this.$message.warning("审批人与当前用户不符，无法审批")
      }
    ...
    tableDbClickHandle(row, column, event) {
      ...
      this.$nextTick(() => {
        ...
        this.$refs.approvalStatus.init(row.purchaseId);
      });
    },
