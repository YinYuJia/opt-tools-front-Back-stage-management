<template>
  <el-dialog
    ...
    <el-form-item label="初审备注" prop="firstRemark">
        <el-input :disabled="!showApproval" v-model="dataForm.firstRemark"/>
      </el-form-item>
    
    <el-form-item label="终审备注" prop="lastRemark">
        <el-input :disabled="true" v-model="dataForm.lastRemark"  />
      </el-form-item>
    </el-form>
...
// --------审批状态----------
      approvalSubmit(status){
        this.$http({
          url: this.$http.adornUrl('/oa/oavehiclemaintain/first/approval'),
          method: 'post',
          data: this.$http.adornData({
            'vehicleMaintainId':this.dataForm.vehicleMaintainId,
            'approvalStatus':status,
            'firstRemark': this.dataForm.firstRemark
          })
        }).then(({data})=>{
          if (data && data.code === 0) {
