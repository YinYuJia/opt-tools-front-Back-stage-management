...
<script>
  import {groupList,doGroupDetele,groupUpdate} from "@/api/modules/sys/group"
    ...
    methods: {
      ...
      deleteHandle (val) {
        this.$confirm('确定对['+ val.gName + '}]进行删除操作?', '提示', {
          ...
          doGroupDetele(val.gCode).then(({data})=>{
            if(data && data.code === 0){
              groupUpdate({'groupId':val.groupId,'gStatus':'0'}).then(({data}) => {
                if (data && data.code === 0) {
                  this.$message({
                    message: '操作成功',
                    type: 'success',
